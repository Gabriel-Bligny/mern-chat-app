...
function MyChats({ fetchAgain }) {
    ...
    const fetchChats = async () => {
        try {
            ...
            const { data } = await axios.get("https://mern-chat-app-backend-22ov.onrender.com/api/chat", config)
            console.log(data)
            setChats(data)
        }
        catch (error) {
    ...
    return (
        <Box display={{ base: selectedChat ? "none" : "flex", md: "flex" }} flexDir="column"
            alignItems="center" p={3} bg="white" w={{ base: "100%", md: "31%" }} borderRadius="lg" borderWidth="1px" >
